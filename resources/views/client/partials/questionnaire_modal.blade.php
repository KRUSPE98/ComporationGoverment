<!-- MODAL CUESTIONARIO AUTODIAGNÓSTICO -->
<div class="modal fade" id="questionnaireModal" tabindex="-1" aria-labelledby="questionnaireModalLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header" style="background-color: #800020; color: #ffffff;">
                <h5 class="modal-title" id="questionnaireModalLabel" style="font-size: 1.1rem; font-weight: 700; color: #fff;">
                    <i class="bi bi-journal-check me-2"></i> Diagnóstico de profesionalización y gobernanza en empresas familiares
                </h5>
            ...
            <div class="modal-body" style="background-color: #fcfcfc;">
                ...
                <form id="questionnaireModalForm" method="POST" action="{{ route('questionnaire.submit') }}">
                    ...
                    <div class="wizard-step-content active" data-step="1">
                        ...
                        <div class="card p-3 border-warning bg-light mb-3">
                            ...
                            @foreach($questionnaire['privacy_notice']['paragraphs'] as $pText)
                                <p class="small text-dark mb-2 text-lowercase-first" style="color: #212529 !important; text-transform: none !important;">{{ $pText }}</p>
                            @endforeach
                            <p class="small text-muted italic my-2" style="text-transform: none !important;">{{ $questionnaire['privacy_notice']['disclaimer'] }}</p>
                            <div class="mt-3 p-2 bg-white rounded border d-flex align-items-center">
                                <input class="form-check-input my-0" type="checkbox" name="privacy_accepted" value="1" id="privacy_accepted_modal" required style="width: 20px !important; height: 20px !important; min-width: 20px !important; margin-right: 12px !important; margin-left: 0 !important; float: none !important; opacity: 1 !important; visibility: visible !important; cursor: pointer;">
                                <label class="form-check-label font-weight-bold text-dark my-0" for="privacy_accepted_modal" style="color: #212529 !important; font-size: 0.95rem; cursor: pointer; text-transform: none !important;">
                                    {{ $questionnaire['privacy_notice']['checkbox_label'] }} <span class="text-danger">*</span>
                                </label>
                            </div>
                            <div class="text-danger small mt-2 d-none" id="privacy_modal_error">
                                <i class="bi bi-exclamation-triangle-fill me-1"></i> Debe aceptar el aviso de privacidad para continuar.
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="wizard-step-content" data-step="2">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-12">
                                ...
                                <select class="form-select select-relationship" name="company_relationship" required data-target="#relationship_other_modal_wrapper">
                            ...
                            <div class="col-md-12 d-none" id="relationship_other_modal_wrapper">
                    ...
                    <div class="wizard-step-content" data-step="3">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-12">
                                ...
                                <select class="form-select select-generations" name="generations_interacting" required data-target="#generations_other_modal_wrapper">
                                    <option value="">-- Seleccione una opción --</option>
                            ...
                            <div class="col-md-12 d-none" id="generations_other_modal_wrapper">
                    ...
                    <!-- STEP 4: INSTRUCCIONES DEL AUTODIAGNÓSTICO -->
                                ...
                                <div class="scale-container">
                                    @for($i = 0; $i <= 10; $i++)
                                        <div class="scale-option">
                                            <input type="radio" id="modal_m_part1_q{{ $qNum }}_{{ $i }}" name="part1_q{{ $qNum }}" value="{{ $i }}" {{ $i == 0 ? 'checked' : '' }}>
                                            <label for="modal_m_part1_q{{ $qNum }}_{{ $i }}">{{ $i }}</label>
                                        </div>
                                    @endfor
                                </div>
                            </div>
                        ...
                        @foreach($questionnaire['sections']['part2']['questions'] as $qNum => $qText)
                            <div class="question-card">
                                ...
                                <div class="scale-container">
                                    @for($i = 0; $i <= 10; $i++)
                                        <div class="scale-option">
                                            <input type="radio" id="modal_m_part2_q{{ $qNum }}_{{ $i }}" name="part2_q{{ $qNum }}" value="{{ $i }}" {{ $i == 0 ? 'checked' : '' }}>
                                            <label for="modal_m_part2_q{{ $qNum }}_{{ $i }}">{{ $i }}</label>
                                        </div>
                                    @endfor
                                </div>
                            </div>
                        ...
                        @foreach($questionnaire['sections']['part3']['questions'] as $qNum => $qText)
                            <div class="question-card">
                                ...
                                <div class="scale-container">
                                    @for($i = 0; $i <= 10; $i++)
                                        <div class="scale-option">
                                            <input type="radio" id="modal_m_part3_q{{ $qNum }}_{{ $i }}" name="part3_q{{ $qNum }}" value="{{ $i }}" {{ $i == 0 ? 'checked' : '' }}>
                                            <label for="modal_m_part3_q{{ $qNum }}_{{ $i }}">{{ $i }}</label>
                                        </div>
                                    @endfor
                                </div>
                            </div>
